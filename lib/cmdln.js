...
function Cmdln(config) {
    ...
    if (self.helpSubcmds !== null) {
        ...
        self.helpSubcmds.forEach(function (sc) {
            if (typeof (sc) === 'object') {
                assert.string(sc.group, 'subcmdOrder.group');
                assert.optionalBool(sc.unmatched, 'subcmdOrder.unmatched');

                self._subcmdOrder.push(sc);

                if (sc.unmatched) {
                    if (++unmatchCount > 1) {
                        throw (new Error('"unmatched" directive used more ' +
                            'than once in "subcmdOrder" option'));
                    }
        ...
        if (unmatchedNames.length > 0) {
            throw (new Error('unmatched command handlers found: ' +
                unmatchedNames.join(', ') + '.'));
        }
    } else {
